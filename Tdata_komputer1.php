...
$sql = "SELECT * FROM barang WHERE barcode='$_POST[barcode]'";
  $query = mysqli_query($koneksi, $sql) or die (mysql_error());
 
  if(mysqli_num_rows($query) > 0){
    
    echo "<script type='text/javascript'>swal('Gagal Menambahkan Data!','Maaf Code Sudah Ada Boss', 'warning',);</script>";
 
  } else { 

          if(!$insert = mysqli_query($koneksi, "INSERT INTO barang VALUES (NULL,'$barcode','$nama','$harga_beli', '$stock')")){
              echo "<script type='text/javascript'>swal('Gagal Menambahkan Data!','', 'warning',);</script>";
            }else{
 ...
 <hr>
        ...
        <align><center> <a href="tables2.php" class="btn btn-sm btn-primary"><i class="fas fa-backward"></i> KEMBALI KE DATA BARANG</a></center></align>
        <br>
   </body>
 </div>
</div>
